# Graph Report - ai-worker  (2026-06-21)

## Corpus Check
- 75 files · ~97,302 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1305 nodes · 1441 edges · 65 communities (57 shown, 8 thin omitted)
- Extraction: 100% EXTRACTED · 0% INFERRED · 0% AMBIGUOUS
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `81b9674b`
...
1. `AI Worker — Backend Implementation Plan` - 16 edges
2. `useWorkspace()` - 13 edges
3. `AI Worker — Backend Architecture Blueprint` - 12 edges
4. `Phase 5 — Chat System & AI Agent (Core Loop)` - 11 edges
5. `Phase 0 — Project Foundation & Infrastructure` - 9 edges
6. `Phase 4 — Tool System Architecture` - 9 edges
7. `Phase 7 — Workflow Engine` - 9 edges
8. `7.2 Endpoints` - 9 edges
9. `Phase 10 — Billing Readiness & Production Hardening` - 8 edges
10. `3. Database Design` - 8 edges
...
- `IntegrationsPage()` --calls--> `useWorkspace()`  [EXTRACTED]
  apps/web/src/app/(dashboard)/integrations/page.tsx → apps/web/src/context/workspace-context.tsx
- `ConnectedGithubPage()` --calls--> `useWorkspace()`  [EXTRACTED]
  apps/web/src/app/(dashboard)/integrations/github/page.tsx → apps/web/src/context/workspace-context.tsx
- `SettingsPage()` --calls--> `useWorkspace()`  [EXTRACTED]
  apps/web/src/app/(dashboard)/settings/page.tsx → apps/web/src/context/workspace-context.tsx
- `connectGitHubHandler()` --calls--> `createGitHubOAuthUrl()`  [EXTRACTED]
  apps/api/src/features/integrations/github/github.controller.ts → apps/api/src/features/integrations/github/github.service.ts
...
Cohesion: 0.07
Nodes (46): adapter, prisma, Conversation, Integration, Memory, Message, PrismaClient, ToolExecution (+38 more)
...
Cohesion: 0.14
Nodes (16): CreateWorkspaceModal(), CreateWorkspaceModalProps, DashboardSidebar(), navItems, Integration, useWorkspace(), Workspace, WorkspaceContext (+8 more)
...
Cohesion: 0.09
Nodes (21): DateTimeFilter, DateTimeWithAggregatesFilter, JsonNullableFilter, JsonNullableFilterBase, JsonNullableWithAggregatesFilter, JsonNullableWithAggregatesFilterBase, NestedDateTimeFilter, NestedDateTimeWithAggregatesFilter (+13 more)
...
Cohesion: 0.14
Nodes (14): 9.1 Audit Logs, 9.2 RBAC Enforcement, 9.3 Rate Limiting, 9.4 Security Hardening, 9.5 Verification Checklist, `audit.service.ts` — Implementation:, code:block88 (apps/api/src/modules/audit/), code:block89 (log(event: AuditEvent):) (+6 more)

### Community 25 - "Community 25"
Cohesion: 0.14
Nodes (14): 10.1 Billing Module (Stripe-Ready Stubs), 10.2 Health & Monitoring Endpoints, 10.4 Graceful Shutdown, 10.5 Dockerfile, 10.6 Environment Template, 10.7 Verification Checklist, code:block100 (Handle SIGTERM / SIGINT:), code:dockerfile (# Multi-stage build:) (+6 more)
...
Cohesion: 0.25
Nodes (8): 8.1 Database — Automations Table, 8.3 Automation Scheduler, 8.4 API Endpoints, 8.5 Verification Checklist, code:block85 (On server startup:), code:block86 (POST   /automations                  → Create automation), code:block87 (□ Create automation with valid cron → registers repeatable j), Phase 8 — Automation Engine
...
Cohesion: 0.5
Nodes (4): 8.2 Automations Module, `automations.service.ts` — Methods:, code:block83 (apps/api/src/modules/automations/), code:block84 (- create(workspaceId, data: CreateAutomationDto): Automation)

### Community 44 - "Community 44"
Cohesion: 0.67
Nodes (3): 10.3 Error Tracking, code:bash (pnpm add @sentry/node), code:block99 (Initialize Sentry in index.ts:)

## Knowledge Gaps
- **974 isolated node(s):** `app`, `adapter`, `GitHubRouter`, `githubScopes`, `integrationSelect` (+969 more)
...
- **Why does `AI Worker — Backend Implementation Plan` connect `Community 28` to `Community 33`, `Community 10`, `Community 12`, `Community 17`, `Community 18`, `Community 19`, `Community 20`, `Community 21`, `Community 22`, `Community 24`, `Community 25`?**
  _High betweenness centrality (0.015) - this node is a cross-community bridge._
- **Why does `Phase 6 — BullMQ Job Infrastructure` connect `Community 21` to `Community 28`?**
  _High betweenness centrality (0.004) - this node is a cross-community bridge._
- **Why does `Phase 8 — Automation Engine` connect `Community 33` to `Community 41`, `Community 28`?**
  _High betweenness centrality (0.004) - this node is a cross-community bridge._
- **What connects `app`, `adapter`, `GitHubRouter` to the rest of the system?**
