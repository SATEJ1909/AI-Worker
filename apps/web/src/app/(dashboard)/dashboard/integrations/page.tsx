...
import { GitBranch, Plus, CheckCircle2, AlertCircle, Lock, ExternalLink, Calendar, Server, Trash2 } from 'lucide-react';
...
const INTEGRATION_PROVIDERS = [
  ...
  },
  {
    providerId: 'gmail',
    name: 'Gmail',
    description: 'Search threads, summarize emails, draft replies, and route approvals',
    authType: 'OAuth',
    icon: GmailIcon,
    available: false,
  },
  {
    providerId: 'drive',
    name: 'Google Drive',
    description: 'Find files, cite context, and extract answers from documents',
    authType: 'OAuth',
  ...
  },
  {
    ...
    description: 'Check availability, schedule meetings, and resolve time conflicts',
    authType: 'OAuth',
    icon: Calendar,
    available: false,
  },
        ...
        <div className="border border-border rounded-xl overflow-hidden bg-card/40 shadow-sm">
          ...
          <div className="divide-y divide-border">
            {INTEGRATION_PROVIDERS.map((provider) => {
              const isConnected = activeWorkspace?.integrations?.some((int: any) => int.provider === provider.providerId);
